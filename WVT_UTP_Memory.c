 ...
 * @return WVT_UTP_ERROR если структура не найдена, WVT_UTP_OK если структура успешно прочитана
 */
WVT_UTP_Status_t WVT_UTP_Memory_get_header_by_serial_number(WVT_UTP_Data_Header_t *header, uint16_t sn){
    if(((sn+1)*sizeof (WVT_UTP_Data_Header_t))>utp_struct.memory_size) return WVT_UTP_ERROR;
    uint16_t crc = 0;
    ...
    crc = WVT_UTP_Memory_update_crc16(crc, (uint8_t *)header, sizeof(WVT_UTP_Data_Header_t)-sizeof(header->crc_of_this_struct));
    if(crc != header->crc_of_this_struct) return WVT_UTP_ERROR;
    return WVT_UTP_OK;
}
 ...
 * @return WVT_UTP_ERROR если область зарезервировать не удалось, WVT_UTP_OK при успешном резервировании
 */
 ...
 * @return WVT_UTP_ERROR если область очистить не удалось, WVT_UTP_OK при успешной очистке
 */
WVT_UTP_Status_t WVT_UTP_Memory_free(WVT_UTP_Data_Header_t *header){
 ...
 * @return WVT_UTP_ERROR если область записать не удалось, WVT_UTP_OK при успешной записи
 */
WVT_UTP_Status_t WVT_UTP_Memory_write(WVT_UTP_Data_Header_t *header, uint32_t offset, uint8_t *buffer, uint32_t length)
{
    if((length+offset) > header->size) return WVT_UTP_ERROR;
